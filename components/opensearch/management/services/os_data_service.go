...
func AppendExcludeNodeHost(service *OsClusterClient, nodeNameToExclude string) (bool, error) {
	...
	val, ok := FindByPath(response.Transient, ClusterSettingsExcludeBrokenPath)
...
func PreparePodForDelete(service *OsClusterClient, lg logger.Logger, podName string, drainNode bool, nodeCount int32) (bool, error) {
	if drainNode {
		...
		if nodeCount == 2 {
			...
			lg.Infof("Waiting to drain primary replicas for system indices from node %s before deleting", podName)
		...
		lg.Infof("Waiting for node %s to drain before deleting", podName)
...
func ShouldUpdateIndexTemplate(
	...
	if indexTemplateResponse.IndexTemplate.Template.Settings != nil {
		indexTemplateResponse.IndexTemplate.Template.Settings, err = SortedJsonKeys(indexTemplateResponse.IndexTemplate.Template.Settings)
		if err != nil {
			return false, err
		}
	}
	...
	if indexTemplateResponse.IndexTemplate.Template.Mappings != nil {
		indexTemplateResponse.IndexTemplate.Template.Mappings, err = SortedJsonKeys(indexTemplateResponse.IndexTemplate.Template.Mappings)
		if err != nil {
			return false, err
		}
	}
