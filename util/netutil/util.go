...
var (
	vPathTCPTab  = pathTCPTab
	vPathTCP6Tab = pathTCP6Tab
	vPathUDPTab  = pathUDPTab
	vPathUDP6Tab = pathUDP6Tab
)

func SetPathTCPTab(path string) {
	vPathTCPTab = path
}

func SetPathTCP6Tab(path string) {
	vPathTCP6Tab = path
}

func SetPathUDPTab(path string) {
	vPathUDPTab = path
}

func SetPathUDP6Tab(path string) {
	vPathUDP6Tab = path
}
...
func osTCPSocks(accept AcceptFn) ([]SockTabEntry, error) {
	return doNetstat(vPathTCPTab, accept)
}
...
func osUDP6Socks(accept AcceptFn) ([]SockTabEntry, error) {
	return doNetstat(vPathUDP6Tab, accept)
}
